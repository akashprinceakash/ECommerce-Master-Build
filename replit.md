...
- `src/components/3d/gt-styles.ts` — GT Design Style System (GT001–GT032). **Programmatic painter engine**: each style is defined as a list of `DrawLayer` instructions (`fillZone`, `sidePanels`, `wavePanels`, `sleeveBand`, `hemBand`, `hourglassPanel`, `raglanYoke`, `pinstripes`). `renderTexture()` paints those layers onto a native `<canvas>` at 1024×1024 using `CanvasRenderingContext2D` (`fillRect`, `bezierCurveTo`) — coordinates are pixel-perfect with the UV map (no Fabric scaling artefacts). `UV_ZONES` defines the bounding box of each UV island (front, back, collar, leftSleeve, rightSleeve). The resulting PNG dataURL is loaded as a Fabric `FabricImage` covering the whole canvas at `left:0, top:0, scaleX:1, scaleY:1`. The image is tagged `data.tag === "__kashaGtBg__"`, sent to back, and `clearGtStyle()` removes only that tag — text/logos/prints stay on top. **Recoloring** = re-render the texture with new colors and swap the FabricImage (no pixel-walk, no fringing, instant). The new image is added BEFORE the old one is removed to avoid a flash. `customize.tsx` awaits `applyGtStyle` before calling `syncTexture()` and uses a monotonic `gtRequestIdRef` token to avoid race conditions on rapid style/color clicks. Verified with pixel-sampling tests: GT001 body=#C5D3DE/collar=#362223; GT006 olive centre + black curved side panels + black sleeves; GT017 slate hourglass centre + black edges; quick-palette swatch click recolors only the targeted role.
