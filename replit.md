...
- `src/components/3d/gt-styles.ts` — GT Design Style System (GT001–GT032). **Hybrid engine**: `applyGtStyle()` dispatches by `style.group`. The **Classic** group (GT001–GT005) uses **live zone-based rendering** — `classicLayout()` builds a list of Fabric `Rect` specs derived from `ZONE_PRESETS` (in `patterns.ts`): one `primary`-filled rect per body UV island (front, back, leftSleeve, rightSleeve), plus `accent`-filled trim rects on the collar band (= full `Z.collar`), the bottom HEM_H px of front + back, the bottom CUFF_H px of each sleeve, and a PLACKET_W × PLACKET_H vertical strip at the centre-top of the front. Trim band sizes are tunable constants at the top of the file (HEM_H=60, CUFF_H=32, PLACKET_W=48, PLACKET_H=220). Each rect is `selectable:true, evented:true` and tagged `data: { tag:"__kashaGtBg__", styleId, layer:"body"|"trim" }`, so individual bands are draggable on the CANVAS tab and `clearGtStyle()` wipes them in one pass. Stack ordering: trim rects are sent to back first, then body rects, so body ends up at the very back and trim sits just above it — both below prints/text/logos. Recolour is instant (no pixel walk) because we just rebuild the rects with the new colours. **All other groups** (sport-side, triple, wave, hourglass, pinstripe, raglan) still use the **legacy texture path**: each style is a pre-baked 1024×1024 PNG in `GT_BASE_TEXTURES` (base64), and `recolorTexture()` pixel-walks an offscreen canvas to swap default primary/accent/tertiary RGB values (Manhattan distance < 10) before pasting it as one full-canvas Fabric image. These groups will be migrated to zone-based recipes once the client confirms the layout per group. `customize.tsx` awaits `applyGtStyle` before calling `syncTexture()` and uses a monotonic `gtRequestIdRef` token to avoid race conditions on rapid style/colour clicks.
