...
- `src/components/3d/gt-styles.ts` — GT Design Style System (GT001–GT032). **Texture-based engine**: each style is a pre-rendered 1024×1024 PNG (base64-embedded in `GT_BASE_TEXTURES`). `applyGtStyle()` loads the PNG as a Fabric `FabricImage` covering the whole canvas at `left:0, top:0, scaleX:1, scaleY:1` — since the texture and UV space are both 1024×1024, alignment is perfect every time (no shape-coordinate math). `recolorTexture()` pixel-walks an offscreen canvas to swap default primary/accent/tertiary RGB values (Manhattan distance < 8) for customer-chosen colors in <100ms. The image is tagged `data.tag === "__kashaGtBg__"`, sent to back, and `clearGtStyle()` removes only that tag — text/logos/prints stay on top. `customize.tsx` awaits `applyGtStyle` before calling `syncTexture()` and uses a monotonic `gtRequestIdRef` token to avoid race conditions on rapid style/color clicks.
...
- **GT Classic group (GT001–GT005) UV zoning** (April 2026): the baked Classic textures were regenerated to match a true polo-trim layout — body (front + back + sleeves) is one uniform primary colour, and the accent colour appears ONLY on thin trim strips: collar binding edge (top of the collar UV zone), placket strip (front-centre vertical), cuff edge ring (outer edge of each sleeve UV), and bottom hem (bottom strip of front + back). Previously these textures had large rectangular UV zones flooded with accent that mapped to shoulder/upper-sleeve panels on the GLB and made the shirt look colour-blocked instead of trim-accented. The other groups (Sport Side, Triple Tone, Wave, Hourglass, Pinstripe, Raglan) intentionally keep their multi-zone block-colour designs. Textures are regenerated by running `pnpm --filter @workspace/scripts exec tsx ./src/gen-gt-classic-textures.ts` (writes new base64 strings to `/tmp/gt-classic-out.txt`); paste them back into `GT_BASE_TEXTURES` in `gt-styles.ts`. The pixel-swap recolour engine still works on the new textures because the same primary/accent default hexes are used.

- **Left panel**: Garment Parts — fixed 5-zone list (Front, Back, Left Sleeve, Right Sleeve, Collar — no Full Body). Each row has its own inline `<input type="color">` so the customer can pick a colour for that zone directly without a separate palette section. Each zone is painted onto the texture canvas as a `fabric.Rect` at the matching `ZONE_PRESETS` rectangle and tagged `data.kashaZoneColor === <zone>` so it can be replaced/cleared cleanly. Stack order: GT base → zone-colour rects → prints → text/logos/shapes. Below: Auto Rotate toggle, Size (XS–XXL), Quantity, Save/Export. The standalone "Color Palette" block was removed — colours live inline in the Garment Parts rows. The single source of truth for placement / zone coords is `patterns.ts → ZONE_PRESETS`; `PLACEMENTS` (used by text / logo / shape "Front Chest / Front Center / Back Top / Back Center / Sleeve Left / Sleeve Right" buttons) is derived from those same rectangles via the `_zCenter()` helper. Text and logos are added with `flipX:true` to cancel the body-UV horizontal mirror so directional content reads correctly on the 3D shirt (typing "Akash" reads "Akash" on the model — without the pre-flip it would render as "hsakA"). Patterns/prints are repeating + roughly symmetric so the mirror isn't visible on them. The GT base layer is now `selectable:true, evented:true` so customers can drag/scale the print on the CANVAS tab too.
