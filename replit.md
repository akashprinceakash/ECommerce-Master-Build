...
- `src/components/3d/gt-styles.ts` — GT Design Style System (GT001–GT032). **Procedural engine** (v3): `renderGtTexture(style, colors)` draws each style fresh onto a 1024×1024 offscreen canvas using the same UV-zone coordinates the print library uses (`ZONE_PRESETS` from `patterns.ts`), so designs land on exactly the right parts of the shirt and match the official spec sheets (e.g. GT003 = solid sage body + brown collar + thin brown cuff trim + thin brown bottom hem — no oversized side panels, no random vertical bars). Each `group` has its own draw routine: `classic` = body + collar + thin cuff/hem bands; `sport-side` = body + tapered side panels + outer-half sleeves; `triple` = 3-tone yoke + cuffs + hem; `wave` = wider tapered panels; `hourglass` = vertical centre panel; `pinstripe` = vertical thin stripes + collar + cuffs; `raglan` = full-accent sleeves + collar + thin hem. Colour overrides are applied at draw time (no pixel-walk recolour pass needed). `applyGtStyle()` produces the data URL, loads it as a Fabric `FabricImage` at `left:0, top:0, scaleX:1, scaleY:1` (texture and UV are both 1024×1024, so alignment is exact), tags it `data.tag === "__kashaGtBg__"`, and sends it to back. The image is `selectable:true, evented:true` so the customer can drag/scale/rotate the GT print on the CANVAS tab. `clearGtStyle()` removes only that tag — text/logos/prints stay on top. `customize.tsx` awaits `applyGtStyle` before calling `syncTexture()` and uses a monotonic `gtRequestIdRef` token to avoid race conditions on rapid style/color clicks.
