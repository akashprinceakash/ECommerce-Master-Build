...
- Background-removed garment product photos must be cropped to their tight non-transparent bounding box before being placed in the anchor box. Catalog photos have wildly inconsistent padding around the garment; without cropping, `object-fit: contain` centers each garment at a different scale/position, so the same anchor box makes some shirts render too small/offset and the avatar's baked-in default outfit peeks through the gap (visible as a white sliver at the collar/shoulders).
