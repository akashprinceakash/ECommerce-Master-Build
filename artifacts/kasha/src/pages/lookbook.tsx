...
export default function LookbookPage() {
  ...
  const editorialItems = useMemo(
    () => allProducts.filter(p => p.thumbnailUrl).sort((a, b) => a.id - b.id),
    [allProducts],
  );
  ...
  return (
    <Layout>
      ...
      <section style={{ background: "#F9F8F6", padding: "64px 24px 0", textAlign: "center" }}>
        <p style={{ fontFamily: FONT_UI, fontSize: 10, letterSpacing: "0.45em", color: GOLD, textTransform: "uppercase", marginBottom: 20 }}>
          SS 2026 Collection
        </p>
        <p style={{ fontFamily: FONT_DISPLAY, fontSize: "clamp(17px, 2vw, 22px)", color: "rgba(0,0,0,0.55)", maxWidth: 680, margin: "0 auto", lineHeight: 1.8, letterSpacing: "0.02em" }}>
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto" }}>
          {editorialItems.length > 0 && (
            <div className="columns-2 md:columns-3 lg:columns-4 gap-4">
              {editorialItems.map((product, i) => {
                const imgSrc = getAssetUrl(product.thumbnailUrl) || "";
                return (
                  <Link key={product.id} href={`/products/${product.id}`}>
                    <div
                      className="group"
                      style={{ breakInside: "avoid", marginBottom: 16, position: "relative", overflow: "hidden", cursor: "pointer", background: "#EDEBE6" }}
                    >
                      <img
                        src={imgSrc}
                        alt={product.name}
                        ...
                        decoding="async"
                        style={{ width: "100%", display: "block", objectFit: "contain", transition: "transform 0.6s ease" }}
                        className="group-hover:scale-105"
                      />
                      <div
                        className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                        style={{ background: "linear-gradient(to top, rgba(0,0,0,0.72) 0%, rgba(0,0,0,0.1) 55%, transparent 100%)", display: "flex", alignItems: "flex-end", padding: "16px 14px" }}
                      >
                        <div>
                          <p style={{ fontFamily: FONT_UI, fontSize: 12, letterSpacing: "0.1em", color: "#fff", marginBottom: 3 }}>
                            {product.name.replace(/\s+[—–-]\s*[A-Z]{1,3}\d+\s*$/, "")}
                          </p>
                          <p style={{ fontFamily: FONT_UI, fontSize: 11, letterSpacing: "0.08em", color: GOLD }}>
                            {formatPrice(product.priceInPaise)} &nbsp;→
                          </p>
                        </div>
                      </div>
                      {i % 7 === 0 && (
                        <div style={{ position: "absolute", top: 12, left: 12, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(6px)", padding: "4px 10px" }}>
                          <span style={{ fontFamily: FONT_UI, fontSize: 9, letterSpacing: "0.35em", color: GOLD, textTransform: "uppercase" }}>SS 2026</span>
                        </div>
                      )}
                    </div>
                  </Link>
          ...
          {editorialItems.length > 0 && (
            <div style={{ textAlign: "center", marginTop: 56 }}>
              <Link href="/products" style={{ display: "inline-block", background: GOLD, color: "#fff", fontFamily: FONT_UI, fontSize: 12, letterSpacing: "0.3em", textTransform: "uppercase", padding: "16px 48px", textDecoration: "none" }}>
                Shop the Collection
              </Link>
            </div>
          )}
