...
const LOGO_POSITIONS: Record<string, { left:number; top:number }> = {
  ...
  "collar-left":   { left: 115, top: 210 },  // left collar flap — near outer seam, left quarter of collar zone
  "collar-right":  { left: 400, top: 210 },  // right collar flap — near outer seam, right quarter of collar zone
};
// Collar placements need a -90° rotation so logos/text run horizontally along each flap.
const LOGO_ANGLES: Record<string, number> = {
  "collar-left":  -90,
  "collar-right": -90,
};
function placementAngle(placement: string): number {
  return LOGO_ANGLES[placement] ?? 0;
}
...
// No vertical flip needed for any current placement.
...
export default function CustomizePage() {
  ...
  const removeBackground = useCallback(async () => {
    ...
    try {
      ...
      if (logoObjRef.current&&fcRef.current) {
        ...
        ni.set({left:pos.left,top:pos.top,originX:"center",originY:"center",angle:placementAngle(logoPosition),flipX:placementFlipX(logoPosition),flipY:placementFlipY(logoPosition)});
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload=async(ev)=>{
      ...
      img.set({left:pos.left,top:pos.top,originX:"center",originY:"center",angle:placementAngle(logoPosition),flipX:placementFlipX(logoPosition),flipY:placementFlipY(logoPosition)});
      const fc=fcRef.current; if(!fc) return;
  ...
  const repositionLogo = () => {
    ...
    const maxW=Math.round(logoSize*(1024/100));
    o.scaleToWidth(maxW);
    o.set({left:pos.left,top:pos.top,originX:"center",originY:"center",angle:placementAngle(logoPosition),flipX:placementFlipX(logoPosition),flipY:placementFlipY(logoPosition)});
    o.setCoords();
  ...
  const applyText = () => {
    ...
    const txt=new (fabric as any).IText(textInput.trim(),{
      ...
      textAlign:textAlign,
      angle:placementAngle(textPosition),flipX:placementFlipX(textPosition),flipY:placementFlipY(textPosition),
  ...
  const repositionText = () => {
    ...
    const pos=LOGO_POSITIONS[textPosition]||{left:512,top:512};
    o.set({left:pos.left, top:pos.top, originX:"center", originY:"center", angle:placementAngle(textPosition),flipX:placementFlipX(textPosition),flipY:placementFlipY(textPosition), fontSize:textFontSize, fill:textColor, fontFamily:textFont, fontWeight:textBold?"700":"400", fontStyle:textItalic?"italic":"normal"});
    o.setCoords();
    fcRef.current?.renderAll(); syncTexture();
  };
                      ...
                      return(
                        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:5}}>
                          {cards.map(c=>{
                            ...
                            const svg=`<svg viewBox="0 0 60 68" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M22 4L10 12L4 32L14 34L14 64H46L46 34L56 32L50 12L38 4L34 6C32 8 28 8 26 6Z" fill="#e8e4dc" stroke="#1a1a18" stroke-width="1.5"/>${c.back?`<text x="30" y="54" text-anchor="middle" font-size="6" fill="#999" font-family="sans-serif">back</text>`:""}<circle cx="${c.cx}" cy="${c.cy}" r="3.5" fill="${isA?"#c9a84c":"#aaa"}"/></svg>`;
                            return(
                              <div key={c.key} onClick={()=>{setLogoPosition(c.key);setCameraView(PLACEMENT_VIEW[c.key]||"front");setModelPaused(true);const mv_=mvRef.current as any;if(mv_){mv_.removeAttribute("auto-rotate");mv_.removeAttribute("auto-rotate-delay");}if(logoObjRef.current){const pos=LOGO_POSITIONS[c.key]||{left:512,top:512};logoObjRef.current.set({left:pos.left,top:pos.top,originX:"center",originY:"center",angle:placementAngle(c.key),flipX:placementFlipX(c.key),flipY:placementFlipY(c.key)});logoObjRef.current.setCoords();fcRef.current?.renderAll();syncTexture();}}} style={{
                                display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer",
                                padding:"8px 4px",borderRadius:9,transition:"all .18s",
                                border:`1.5px solid ${isA?V.ac:V.bd}`,background:isA?V.aclt:"transparent",
                              }}>
                  ...
                  <>
                    {logoPreview&&(
                      <div>
                        ...
                        {chipGrid(
                          k=>logoPosition===k,
                          k=>{setLogoPosition(k as any);setCameraView(PLACEMENT_VIEW[k]??"front");setModelPaused(true);const mv_=mvRef.current as any;if(mv_){mv_.removeAttribute("auto-rotate");mv_.removeAttribute("auto-rotate-delay");}if(logoObjRef.current){const pos=LOGO_POSITIONS[k]||{left:512,top:512};logoObjRef.current.set({left:pos.left,top:pos.top,originX:"center",originY:"center",angle:placementAngle(k),flipX:placementFlipX(k),flipY:placementFlipY(k)});logoObjRef.current.setCoords();fcRef.current?.renderAll();syncTexture();}}
                        )}
                      </div>
                    )}
                    {textPlaced&&(
                      <div>
                        ...
                        {chipGrid(
                          k=>textPosition===k,
                          k=>{setTextPosition(k as any);setCameraView(PLACEMENT_VIEW[k]??"front");setModelPaused(true);const mv_=mvRef.current as any;if(mv_){mv_.removeAttribute("auto-rotate");mv_.removeAttribute("auto-rotate-delay");}if(textObjRef.current){const pos=LOGO_POSITIONS[k]||{left:512,top:512};textObjRef.current.set({left:pos.left,top:pos.top,originX:"center",originY:"center",angle:placementAngle(k),flipX:placementFlipX(k),flipY:placementFlipY(k)});textObjRef.current.setCoords();fcRef.current?.renderAll();syncTexture();}}
                        )}
                      </div>
                    )}
